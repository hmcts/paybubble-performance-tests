...
import uk.gov.hmcts.paybubble.util.Common._
...
object RefundsV2 {
  ...
  val getRefunds =
    ...
    group("Refunds") {
        exec(session =>  {
          session.set("refundStartDate", currentDateTimeMinus1Day("yyyy-MM-dd") ).set("refundEndDate", currentDateTime("yyyy-MM-dd"))
        })
