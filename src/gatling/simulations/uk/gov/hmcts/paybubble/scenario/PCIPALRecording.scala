...
object PCIPALRecording extends Simulation {
	...
	val telephonyOnlineScenario = scenario("Telephony Online Scenario")
		...
		.exec(http("PaymentAPI${service}_020_BulkScanCases")
			.get("/api/bulk-scan/cases/1235123512351235")
			.headers(headers_13)
			.check(status.is(200)))
		.pause(thinkTime)

	  .exec(http("PaymentAPI${service}_030_PaymentHistory1")
		...
		.exec(http("PaymentAPI${service}_090_PaymentHistory3")
			.get("/api/payment-history/bulk-scan-feature")
			.headers(headers_11)
			.check(status.is(200)))
		  .pause(thinkTime)

		.exec(http("PaymentAPI${service}_100_PaymentHistory4")
			.get("/api/payment-history/payment-groups/2021-1611244599731")
			.headers(headers_13)
			.check(status.is(200)))
		  .pause(thinkTime)

		.exec(http("PaymentAPI${service}_110_PaymentHistory5")
		...
		.exec(http("PaymentAPI${service}_120_PCIPALThirdCall")
			.get("/pcipalThirdCall")
			.headers(headers_63)
			.check(status.is(200)))
		  .pause(thinkTime)

		.exec(http("PaymentAPI${service}_130_PCIPALStaging1")
		...
		.exec(http("PaymentAPI${service}_140_PCIPALStaging2")
		...
		.exec(http("PaymentAPI${service}_150_PCIPALStaging3")
